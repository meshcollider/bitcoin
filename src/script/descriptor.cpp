...
class StringMaker {
    ...
    StringType m_type;
public:
    StringMaker(const SigningProvider* arg, const std::vector<std::unique_ptr<PubkeyProvider>>& pubkeys, StringType type) : m_arg(arg), m_pubkeys(pubkeys), m_type(type) {}
    bool ToString(uint32_t key, std::string& ret) const { return KeyToString(m_arg, ret, m_type, *m_pubkeys[key]); }
};
...
protected:
    std::vector<CScript> MakeScripts(const std::vector<CPubKey>& keys, Span<const CScript> scripts, FlatSigningProvider&) const override { return Vector(m_node->ToScript(ScriptMaker(keys))); }
public:
    bool ToStringHelper(const SigningProvider* arg, std::string& out, StringType type, const DescriptorCache* cache = nullptr) const override { return m_node->ToString(StringMaker(arg, m_pubkey_args, type), out); }
...
std::unique_ptr<PubkeyProvider> InferPubkey(const CPubKey& pubkey, ParseScriptContext, const SigningProvider& provider)
 ...
 struct KeyParser {
     ...
     {
         assert(m_out);
         auto pk = ParsePubkey(0, Span<const char>(&*begin, &*end), ParseScriptContext::P2WSH, *m_out, m_error);
         key = m_keys.size();
        ...
        if (node) {
            ...
            while (!node->IsValid() || !node->IsNonMalleable()) {
                ...
                if (!found_invalid_sub) {
                    std::string err;
                    node->ToString(StringMaker(nullptr, parser.m_keys, StringType::PUBLIC), err);
                    if (!node->IsValid()) {
